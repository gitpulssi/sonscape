...
info "=== SoniXscape Production Installer v3.8 (Ubuntu 24.04 LTS) ==="
info "Dual-Bluetooth + Loopback Fix + Template Output Service + Auto Pair"
...
[Unit]
Description=SoniXscape Bluetooth Output Bridge for %I
...
ExecStart=/bin/bash -c '/usr/bin/bluealsa-aplay --pcm-buffer-time=250000 --pcm-period-time=50000 -D plughw:Loopback,0 ${0#BT_}' %I
Restart=always
RestartSec=2
...
sudo systemctl enable bluealsa sonixscape-bt-agent sonixscape-web sonixscape-audio sonixscape.service

info "=== Installation Summary ==="
info "✓ BlueALSA v3.0.0 with dual-adapter support"
info "✓ ws_audio.py patched with loopback fallback fix"
