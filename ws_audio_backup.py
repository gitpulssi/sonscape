...
# -*- coding: utf-8 -*-
"""
Production-ready SoniXcape server with HTTP REST API + WebSocket support
"""
...
# Channel mapping and other constants from your original code
CHANNEL_MAP = {
...
class PresetManager:
    """Handles CRUD operations for treatment presets"""
    ...
    def save_preset(self, preset: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Validate rows
            if not isinstance(preset['rows'], list):
            ...
            print(f"[PRESET] Saved preset: {preset['name']} ({preset['id']})")
            return True
    ...
    def delete_preset(self, preset_id: str) -> bool:
        ...
        try:
            ...
            if preset_file.exists():
                preset_file.unlink()
                print(f"[PRESET] Deleted preset: {preset_id}")
                return True
            return False
        except Exception as e:
            print(f"[PRESET] Error deleting preset {preset_id}: {e}")
            return False
...
class WebSocketHandler:
    ...
    async def handle_websocket(self, request):
        ...
        print(f"[WS] Client connected from {client_addr}")
        self.clients.add(ws)
        
        try:
            # Send initial state
            await ws.send_str(f"ack:bt-set-mono:{self.player.bt_mono}")
            ...
            print(f"[WS] Client {client_addr} disconnected")
    ...
    async def handle_message(self, ws, message):
        try:
            ...
            if action == "play-selected":
                row = data.get("row")
                if row:
                    self.player.play_row(row)
                await ws.send_str("ack:play-selected")
            
            elif action == "play-all":
                rows = data.get("rows", [])
                if rows:
                    self.player.play_sequence(rows)
                await ws.send_str("ack:play-all")
            
            elif action == "stop":
                self.player.stop()
                await ws.send_str("ack:stop")
            
            elif action == "bt-set-mono":
                mono_flag = bool(data.get("mono", True))
                self.player.bt_mono = mono_flag
                await ws.send_str(f"ack:bt-set-mono:{mono_flag}")
            
            elif action == "set-mix":
                await self.handle_set_mix(ws, data)
            
            else:
                await ws.send_str("error:unknown")
                
        except json.JSONDecodeError:
            await ws.send_str("error:badjson")
        except Exception as e:
            print(f"[WS] Message handling error: {e}")
            await ws.send_str("error:internal")
    
    async def handle_set_mix(self, ws, data):
        try:
            value = max(0, min(100, int(data.get("value", 50))))
            # Apply mixing logic here
            await ws.send_str("ack:set-mix")
        except Exception:
            await ws.send_str("error:bad-mix")
...
async def get_preset_handler(request):
...
async def legacy_list_presets(request):
    """GET /list-presets - Legacy endpoint"""
    return await list_presets_handler(request)

async def legacy_save_preset(request):
    """POST /save-preset - Legacy endpoint"""
    return await save_preset_handler(request)

async def legacy_delete_preset(request):
    """POST /delete-preset - Legacy endpoint"""
    try:
...
def create_app():
    ...
    # Your existing SineRowPlayer initialization
    player = SineRowPlayer()  # You'll need to adapt this from your existing code
    app['player'] = player
    
    ws_handler = WebSocketHandler(player)
    ...
    # API routes
    app.router.add_get('/api/presets', list_presets_handler)
    app.router.add_get('/api/presets/{id}', get_preset_handler)
    app.router.add_post('/api/presets', save_preset_handler)
    app.router.add_delete('/api/presets/{id}', delete_preset_handler)
    
    # Legacy routes for existing frontend
    app.router.add_get('/list-presets', legacy_list_presets)
    app.router.add_post('/save-preset', legacy_save_preset)
    app.router.add_post('/delete-preset', legacy_delete_preset)
...
async def main():
    ...
    print(f"[HTTP] Access the interface at: http://localhost:{HTTP_PORT}")
...
if __name__ == "__main__":
    # Single instance lock (from your original code)
